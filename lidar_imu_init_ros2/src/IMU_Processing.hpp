    ...
    if (imu_need_init_)
    {
        if(!LI_init_done){
            ...
            if (init_iter_num > MAX_INI_COUNT)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("imu_process"), "IMU Initialization Done: Gravity: %.4f %.4f %.4f, Acc norm: %.4f", stat.gravity[0], stat.gravity[1], stat.gravity[2], mean_acc.norm());
