...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  // Extract per-point timestamps from PointCloud2 "timestamp" field (FLOAT64 at offset 18)
  // The driver (LivoxPointXyzrtlt) publishes: x,y,z,intensity,tag,line,timestamp
  // Step size = 26 bytes, timestamp at offset 18
  std::vector<double> pt_timestamps(plsize, 0.0);
  uint32_t point_step = msg->point_step;
  // Find the timestamp field offset
  int ts_offset = -1;
  for (size_t f = 0; f < msg->fields.size(); f++) {
    if (msg->fields[f].name == "timestamp") {
      ts_offset = msg->fields[f].offset;
      break;
    }
  }
  if (ts_offset >= 0) {
    ...
    for (int i = 0; i < plsize; i++) {
      double ts_val;
      memcpy(&ts_val, data_ptr + i * point_step + ts_offset, sizeof(double));
      pt_timestamps[i] = ts_val;
    }
  }

  // Use either point timestamps (if available) or compute from yaw angles
  if (ts_offset >= 0) {
    // --- Timestamp-based mode: works for any mounting orientation ---
    // Find base time (minimum timestamp in this scan)
    double base_time = pt_timestamps[0];
    for (int i = 1; i < plsize; i++) {
      if (pt_timestamps[i] < base_time) base_time = pt_timestamps[i];
    }
    ...
    for (uint i = 0; i < plsize; ++i)
    {
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      // curvature = relative time offset in milliseconds (FAST-LIO2 expects ms, divides by 1000 internally)
      added_pt.curvature = (pt_timestamps[i] - base_time) / 1e6;
    ...
    // --- Fallback: yaw-angle-based mode (original, only works for horizontal mount) ---
